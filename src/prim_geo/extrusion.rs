...
impl Extrusion {
    ...
    //todo 实现Justline
    pub fn gen_spine_wire(&self, mut new_verts: Vec<Vec3>, thick: f32) -> anyhow::Result<Wire> {
        if new_verts.len() != 3 {
        ...
        // dbg!(rot_axis);
        // dbg!(angle.0.to_degrees());
        // dbg!(origin);
        // dbg!(origin.distance(pt0));
    ...
    pub fn gen_fillet_wire(&self, mut new_verts: Vec<Vec3>) -> anyhow::Result<Wire> {
        ...
        let origin_vert = if abs_diff_eq!(r, 0.0) {
            ...
            if abs_diff_eq!(angle, 0.0) { return Err(anyhow!("fillet的两个方向角度不能为0".to_string())); }
        ...
        for i in 1..=ll {
            ...
            if abs_diff_eq!(fradius, 0.0) {
                ...
                // wire.push_back(builder::line(&t_va, &t_vb));
...
impl BrepShapeTrait for Extrusion {
    fn gen_brep_shell(&self) -> Option<Shell> {
        ...
        let mut new_verts = self.verts.iter().map(|x| Vec3::new(x.x, x.y, 0.0)).collect::<Vec<_>>();
        let mut pre_hash = 0;
    ...
    fn get_scaled_vec3(&self) -> Vec3 {
        ...
        Vec3::new(1.0, 1.0, self.height / 100.0)
    }
}
